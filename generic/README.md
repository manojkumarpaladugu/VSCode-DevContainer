...
This image builds directly from Canonical's official Ubuntu 26.04 image and
provides a persistent development environment for both VS Code/Cursor Dev
...
Open the `generic` directory as a Dev Container. The editor builds and attaches
to the `generic-ubuntu` Compose service as Ubuntu's existing non-root `ubuntu`
...
docker compose exec generic-ubuntu bash
docker compose down
```
...
```bash
docker compose up -d --build --force-recreate generic-ubuntu
```
...
this `generic` directory would otherwise both use the project name `generic`.
Give each concurrently running copy a stable, unique project name:

```bash
export COMPOSE_PROJECT_NAME=generic-my-project
docker compose up -d --build
docker compose exec generic-ubuntu bash
docker compose down
```

The explicitly named `generic-ubuntu` image remains shared. Use `--build` after
switching between copies that contain different Dockerfiles.

For a disposable shell instead of the persistent service:

```bash
docker compose run --rm generic-ubuntu bash
```
...
The runtime includes Python, pip, pipx, CMake, pre-commit, Vim, Git, SSH,
curl/wget, zip, and unzip. CMake and pre-commit are installed in separate pipx
environments.
